...
import { U as isFocusable, V as getFirstFocusable } from "./internal-DqPGr-v-.js";
...
import { a as isGroup, h as useDropdownStatus, m as DropdownFooter, n as useAnnouncement, p as getDropdownMinWidth, r as checkOptionValueField } from "./use-highlight-option-CHKivnUu.js";
import { a as prepareOptions, c as InternalToken, d as styles_css_default$2, h as Filter, i as findOptionIndex, m as plain_list_default, n as useNativeSearch, o as virtual_list_default, r as useLoadItems, s as Trigger, t as useSelect } from "./use-select--MSgp-VV.js";
import { t as checkControlled } from "./check-controlled-B0nHWBFl.js";
import "/Users/jrfabito/Documents/Job Search/react-flow/node_modules/@cloudscape-design/components/internal/components/token-list/styles.scoped.css";
import "/Users/jrfabito/Documents/Job Search/react-flow/node_modules/@cloudscape-design/components/multiselect/styles.scoped.css";
//#region node_modules/@cloudscape-design/components/internal/components/token-list/styles.css.js
...
//#endregion
//#region node_modules/@cloudscape-design/components/internal/components/token-list/token-limit-toggle.js
function TokenLimitToggle({ controlId, allHidden, expanded, numberOfHiddenOptions, onClick, i18nStrings = {}, limitShowFewerAriaLabel, limitShowMoreAriaLabel }) {
	...
	return import_react.createElement("button", {
		type: "button",
		className: styles_css_default$1.toggle,
		onClick: handleClick,
		"aria-controls": controlId,
		"aria-expanded": expanded,
		"aria-label": ariaLabel,
		...getAnalyticsMetadataAttribute(analyticsMetadata)
	}, import_react.createElement(InternalIcon, { name: expanded ? "treeview-collapse" : "treeview-expand" }), import_react.createElement("span", { className: styles_css_default$1.description }, description));
}
//#endregion
//#region node_modules/@cloudscape-design/components/internal/components/token-list/index.js
function TokenList({ items, alignment, renderItem, limit, after, i18nStrings, limitShowFewerAriaLabel, limitShowMoreAriaLabel, onExpandedClick = () => void 0 }) {
	...
	const toggle = hasHiddenItems ? import_react.createElement("div", { className: styles_css_default$1[`toggle-container-${alignment}`] }, import_react.createElement(TokenLimitToggle, {
	...
	if (alignment === "inline") return import_react.createElement("div", { className: clsx(styles_css_default$1.root, styles_css_default$1.horizontal) }, hasItems && import_react.createElement("ul", {
		id: controlId,
		className: styles_css_default$1.list
	}, visibleItems.map((item, itemIndex) => import_react.createElement("li", {
		key: itemIndex,
		className: styles_css_default$1["list-item"],
		"aria-setsize": items.length,
		"aria-posinset": itemIndex + 1
	}, renderItem(item, itemIndex)))), toggle, after && import_react.createElement("div", { className: styles_css_default$1.separator }), after);
	return import_react.createElement("div", { className: clsx(styles_css_default$1.root, styles_css_default$1.vertical) }, hasVisibleItems && import_react.createElement("ul", {
		id: controlId,
		className: clsx(styles_css_default$1.list, {
			[styles_css_default$1.vertical]: alignment === "vertical",
			[styles_css_default$1.horizontal]: alignment === "horizontal",
			[styles_css_default$1.grid]: alignment === "horizontal-grid"
		})
	}, visibleItems.map((item, itemIndex) => import_react.createElement("li", {
		key: itemIndex,
		className: styles_css_default$1["list-item"],
		"aria-setsize": items.length,
		"aria-posinset": itemIndex + 1
	}, renderItem(item, itemIndex)))), toggle, after);
}
//#endregion
//#region node_modules/@cloudscape-design/components/internal/hooks/use-list-focus-controller.js
function useListFocusController({ nextFocusIndex, onFocusMoved, listItemSelector, fallbackSelector, showMoreSelector }) {
	const tokenListRef = (0, import_react.useRef)(null);
	(0, import_react.useEffect)(() => {
		if (nextFocusIndex === void 0 || nextFocusIndex === null || tokenListRef.current === null) return;
		...
		for (let activeIndex = 0; activeIndex < tokenElements.length; activeIndex++) if (activeIndex < nextFocusIndex) closestPrevIndex = nextFocusIndex - activeIndex < nextFocusIndex - closestPrevIndex ? activeIndex : closestPrevIndex;
		else closestNextIndex = activeIndex - nextFocusIndex < closestNextIndex - nextFocusIndex ? activeIndex : closestNextIndex;
		...
		if (focusTarget) onFocusMoved(focusTarget.element, focusTarget.id);
	}, [
		nextFocusIndex,
		listItemSelector,
		fallbackSelector,
...
//#endregion
//#region node_modules/@cloudscape-design/components/token-group/internal.js
function InternalTokenGroup({ alignment, items, onDismiss, limit, i18nStrings, disableOuterPadding, limitShowFewerAriaLabel, limitShowMoreAriaLabel, readOnly, isItemReadOnly, __internalRootRef, ...props }) {
	...
	const tokenListRef = useListFocusController({
		...
		listItemSelector: `.${styles_css_default$1["list-item"]}`,
		showMoreSelector: `.${styles_css_default$1.toggle}`
	});
	...
	return import_react.createElement("div", {
		...baseProps,
		className: clsx(baseProps.className, styles_css_default$2.root, hasItems && styles_css_default$2["has-items"], disableOuterPadding && styles_css_default$2["no-padding"]),
		ref: mergedRef
	}, import_react.createElement(TokenList, {
		alignment,
		items,
		limit,
		renderItem: (item, itemIndex) => import_react.createElement(InternalToken, {
			...item,
			label: item.label,
			key: itemIndex,
			onDismiss: () => {
			...
			readOnly: readOnly || (isItemReadOnly === null || isItemReadOnly === void 0 ? void 0 : isItemReadOnly(item)),
			variant: "normal",
			icon: item.iconName || item.iconUrl || item.iconSvg ? import_react.createElement(InternalIcon, {
				name: item.iconName,
				svg: item.iconSvg,
				url: item.iconUrl,
			...
			...item.disabled || readOnly ? {} : getAnalyticsMetadataAttribute({ detail: { position: `${itemIndex + 1}` } })
		}),
		i18nStrings,
		limitShowFewerAriaLabel,
		limitShowMoreAriaLabel,
		onExpandedClick: (isExpanded) => {
			if (isExpanded && limit) setNextFocusIndex(limit);
			else setNextFocusIndex(null);
		}
	}));
}
//#endregion
//#region node_modules/@cloudscape-design/components/multiselect/use-multiselect.js
function useMultiselect({ options, filteringType, filteringResultsText, disabled, statusType, empty, loadingText, finishedText, errorText, noMatch, renderHighlightedAriaLive, selectedOptions, deselectAriaLabel, keepOpen, onBlur, onFocus, onLoadItems, onChange, controlId, ariaLabelId, footerId, filteringValue, setFilteringValue, externalRef, embedded, enableSelectAll, i18nStrings, ...restProps }) {
	...
	if (restProps.recoveryText && !onLoadItems) warnOnce("Multiselect", "`onLoadItems` must be provided for `recoveryText` to be displayed.");
	...
	const selectedValues = (0, import_react.useMemo)(() => new Set(selectedOptions.map((option) => option.value)), [selectedOptions]);
	...
	const toggleAll = () => {
		...
		fireNonCancelableEvent(onChange, { selectedOptions: isAllVisibleSelectableSelected ? selectedOptions.filter((option) => !filteredNonParentOptionValues.has(option.value)) : allNonParentOptions.filter(({ disabled, value }) => selectedValues.has(value) || !disabled && filteredNonParentOptionValues.has(value)) });
	};
	const updateSelectedOption = (0, import_react.useCallback)((option) => {
		...
		} else newSelectedOptions = isAllChildrenSelected([option]) ? unselect([option], newSelectedOptions) : select([option], newSelectedOptions);
		fireNonCancelableEvent(onChange, { selectedOptions: newSelectedOptions });
	}, [
		selectedOptions,
		onChange,
...
//#endregion
//#region node_modules/@cloudscape-design/components/multiselect/styles.css.js
...
//#endregion
//#region node_modules/@cloudscape-design/components/multiselect/internal.js
var InternalMultiselect = import_react.forwardRef(({ options, filteringType, filteringPlaceholder, filteringAriaLabel, filteringClearAriaLabel, ariaRequired, placeholder, disabled, readOnly, ariaLabel, inlineLabelText, selectedOptions, deselectAriaLabel, tokenLimit, i18nStrings, virtualScroll, inlineTokens = false, hideTokens, expandToViewport, tokenLimitShowFewerAriaLabel, tokenLimitShowMoreAriaLabel, __internalRootRef, autoFocus, enableSelectAll, renderOption, ...restProps }, externalRef) => {
	var _a;
	...
	return import_react.createElement("div", {
	...
	}), showTokens && import_react.createElement(InternalTokenGroup, {
		...multiselectProps.getTokenProps(),
		className: styles_css_default.tokens,
		...
		readOnly,
		isItemReadOnly: (item) => item._readOnly
	}), import_react.createElement(ScreenreaderOnly, { id: ariaLabelId }, ariaLabel || inlineLabelText));
});
//#endregion
export { useListFocusController as a, InternalTokenGroup as i, styles_css_default as n, TokenList as o, useMultiselect as r, styles_css_default$1 as s, InternalMultiselect as t };

//# sourceMappingURL=internal-KkFvwysa.js.map