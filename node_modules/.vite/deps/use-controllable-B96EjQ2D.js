...
//#region node_modules/@cloudscape-design/components/internal/hooks/use-controllable/index.js
...
function useControllable(controlledValue, handler, defaultValue, { componentName, changeHandler, controlledProp }) {
	const isControlled = import_react.useState(controlledValue !== void 0)[0];
	import_react.useEffect(() => {
		if (isControlled && handler === void 0) warnOnce(componentName, `You provided a \`${controlledProp}\` prop without an \`${changeHandler}\` handler. This will render a non-interactive component.`);
	}, [
		handler,
		isControlled,
		componentName,
	...
	const [valueState, setValue] = import_react.useState(defaultValue);
	const [valueHasBeenSet, setValueHasBeenSet] = import_react.useState(false);
	const currentUncontrolledValue = valueHasBeenSet ? valueState : defaultValue;
	const setUncontrolledValue = import_react.useCallback((newValue) => {
		setValue(newValue);
		setValueHasBeenSet(true);
	}, [setValue, setValueHasBeenSet]);
	if (isControlled) return [controlledValue, defaultCallback];
	else return [currentUncontrolledValue, setUncontrolledValue];
}
function defaultCallback() {}
//#endregion
export { useControllable as t };

//# sourceMappingURL=use-controllable-B96EjQ2D.js.map