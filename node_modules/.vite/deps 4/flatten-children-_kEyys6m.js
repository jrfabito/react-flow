...
import { y as warnOnce } from "./apply-display-name-YGZUDjdf.js";
...
//#endregion
//#region node_modules/@cloudscape-design/components/internal/vendor/react-keyed-flatten-children/index.js
...
function flattenChildren$1(children, depth = 0, keys = [], componentName) {
	return import_react.Children.toArray(children).reduce((acc, node, nodeIndex) => {
		if (isFragmentWithChildren(node)) {
			...
			acc.push(...flattenChildren$1(node.props.children, depth + 1, keys.concat(node.key || nodeIndex), componentName));
		} else if ((0, import_react.isValidElement)(node)) acc.push((0, import_react.cloneElement)(node, { key: keys.concat(String(node.key)).join(".") }));
		else if (typeof node === "string" || typeof node === "number") acc.push(node);
		return acc;
	}, []);
}
//#endregion
//#region node_modules/@cloudscape-design/components/internal/utils/react-version.js
function getReactMajorVersion() {
	var _a;
	const versionString = (_a = "19.2.7") === null || _a === void 0 ? void 0 : _a.split(".")[0];
	return versionString ? parseInt(versionString, 10) : NaN;
}
//#endregion
//#region node_modules/@cloudscape-design/components/internal/utils/flatten-children.js
/**
...
*/
function flattenChildren(children, componentName) {
	const majorVersion = getReactMajorVersion();
	if (!Number.isNaN(majorVersion) && majorVersion < 19) return flattenChildren$1(children, 0, [], componentName);
	return import_react.Children.toArray(children);
}
//#endregion
export { getReactMajorVersion as n, flattenChildren as t };

//# sourceMappingURL=flatten-children-_kEyys6m.js.map